...
ups2_init = 1  #Default initial ups2
ua2_init = 0.01  #Default initial ua2
...
ups2_fixed = 0 #Fix ups1?. 0: No; 1: Yes

TCBchecked = False #True: data and response files with time column
...
def process_data(data_read, instru_read, skip):
    ...
    print instru_read
    if TCBchecked == False:
	    fm.data_count = np.genfromtxt(data_read, comments = '*', skip_header = int(skip)) #Read data from file
	    fm.instru_count = np.genfromtxt(instru_read, comments = '*', skip_header = int(skip)) #Read data from file
	    fm.max_temp = 50./gain #TCSPC temporal window size: 50ns/gain
	    fm.temp_data = np.arange(0,fm.max_temp,fm.max_temp/float(fm.data_count.size)) #Generate temporal x-axis array    
    else:
	    fm.data = np.genfromtxt(data_read, comments = '*', delimiter = None, skip_header = int(skip)) #Read data from file
	    fm.instru = np.genfromtxt(instru_read, comments = '*', delimiter = None, skip_header = int(skip)) #Read data from file
	    fm.data_count = fm.data[:,1]
	    fm.instru_count = fm.instru[:,1]
	    fm.temp_data = np.asfarray(fm.data[:,0]) #Temporal x-axis array from experimental data file (converted to float (just in case))

    fm.instru_count = fm.instru_count / fm.instru_count.sum() #Normalize response function to area=1
    fm.data_count = fm.data_count / fm.smooth(fm.data_count).max() #Normalize experimental data with maximum smoothed version
...
def write_results(idx, data_read, fileExt):
    ...
    results_file.write("Initial values: ups2 = " + str(ups2_init) + " ua2 = " + str(ua2_init) + " t0 = " + str(t0_init) + "\n\n")
    results_file.write("Results: \n")
    results_file.write("Fitted values: ups2 = " + str(results[idx][1]) + " ua2 = " + str(results[idx][2]) + " t0 = " + str(results[idx][3]) + " baseline corr= " + str(results[idx][4]) + "\n")
...
class MainWindow:
    def __init__(self):
        ...
        builder = gtk.Builder()
        builder.add_from_file("GTKGlade3.glade")
        ...
        self.ups2Init = builder.get_object("ups2Init")
        self.ups2Init.show()
        self.ups2Init.set_text(str(ups2_init))
        
        self.ua2Init = builder.get_object("ua2Init")
        self.ua2Init.show()
        self.ua2Init.set_text(str(ua2_init))
        ...
        self.ups2FitCheck = builder.get_object("ups2FitCheck")
        self.ups2FitCheck.show()

        self.TimeColumnFileCheck = builder.get_object("TimeColumnFileCheck")
        self.TimeColumnFileCheck.show()
        ...
        self.ColumnUps2 = gtk.TreeViewColumn("Ups2", gtk.CellRendererText(), text=4)
        self.Results.append_column(self.ColumnUps2)
        self.ColumnUa2 = gtk.TreeViewColumn("Ua2", gtk.CellRendererText(), text=5)
        self.Results.append_column(self.ColumnUa2)
        self.ColumnT0 = gtk.TreeViewColumn("T0", gtk.CellRendererText(), text=6)
        self.Results.append_column(self.ColumnT0)
        self.ColumnUps2Var = gtk.TreeViewColumn("Ups2 StdD", gtk.CellRendererText(), text=7)
        self.Results.append_column(self.ColumnUps2Var)
        self.ColumnUa2Var = gtk.TreeViewColumn("Ua2 StdD", gtk.CellRendererText(), text=8)
        self.Results.append_column(self.ColumnUa2Var)
    ...
    def ups2Init_changed_cb(self, widget, data=None):
        global ups2_init
        ups2_init = float(self.ups2Init.get_text())
        
    def ua2Init_changed_cb(self, widget, data=None):
        global ua2_init
        ua2_init = float(self.ua2Init.get_text())
    ...
    def ups2FitCheck_toggled_cb(self, widget, data=None):
        global ups2_fixed
        if self.ups2FitCheck.get_active():
            ups2_fixed = 1
        else:
            ups2_fixed = 0
        print ups2_fixed
        
    def TimeColumnFileCheck_toggled_cb(self, widget, data=None):
	"""We will try to open files with two columns, the first of them with time channels"""
	global TCBchecked
        if self.TimeColumnFileCheck.get_active():
	    TCBchecked = True
	    self.Gain.set_text("N/A")
	    self.Gain.set_editable(False)
	else:
	    TCBchecked = False
    ...
    def ExportButton_clicked_cb(self, widget, data=None):
        ...
        if response == gtk.RESPONSE_OK:
            ...
            export_file.write("Id" + "\t" + "Exp. File" + "\t" + "ups2" + "\t" + "ua2" + "\t" + "t0" + "\t" + "ups2StdD" + "\t" + "ua2StdD" + "\t" + "t0StdD" + "\t" + "norm" + "\n")
    ...
    def FitButton_clicked_cb(self, widget, data=None):
        ...
        global file_instru
        global index        
        print file_instru, file_data, file_result, ups2_init, ua2_init, t0_init
        ...
        fm.ro = ro
        fm.n_ref = n_ref
        if t0_fixed == 0 and ups2_fixed == 0:
            init_vals = ups2_init, ua2_init, t0_init, back_init        
        elif t0_fixed == 1 and ups2_fixed == 0:
            init_vals = ups2_init, ua2_init, back_init          
        elif ups2_fixed == 1 and t0_fixed == 0:
            init_vals = ua2_init, t0_init, back_init
        elif t0_fixed == 1 and ups2_fixed == 1:
            init_vals = ua2_init, back_init
        ...
        for filename in file_data: #Loop through the experimental files.             
            ...
            if type_fit == 1: #Slab
                if t0_fixed == 0:
                    ...
                    popt, pcov = optimize.curve_fit(fm.funcion_fiteo_slab, fm.temp_data, fm.data_count, init_vals)
                    Y = fm.funcion_fiteo_slab(fm.temp_data,popt[0],popt[1],popt[2],popt[3])
                elif t0_fixed == 1:
                    print str(type_fit) + " Slab, t0 fixed"            
                    popt, pcov = optimize.curve_fit(lambda t, ups2, ua2, back: fm.funcion_fiteo_slab(t, ups2, ua2, t0_init, back), fm.temp_data, fm.data_count, init_vals)
                    Y = fm.funcion_fiteo_slab(fm.temp_data,popt[0],popt[1],t0_init,popt[2])
            elif type_fit == 2: #Semi-infinite
                if t0_fixed == 0 and ups2_fixed == 0:
                    print str(type_fit) + " Semi-infinite, t0 and ups2 not fixed"
		    popt, pcov = optimize.curve_fit(fm.funcion_fiteo_refl_klm, fm.temp_data, fm.data_count, init_vals)
                    Y = fm.funcion_fiteo_refl_klm(fm.temp_data,popt[0],popt[1],popt[2],popt[3])
                elif t0_fixed == 1 and ups2_fixed == 0:
                    print str(type_fit) + " Semi-infinite, t0 fixed"
                    popt, pcov = optimize.curve_fit(lambda t, ups2, ua2, back: fm.funcion_fiteo_refl_klm(t, ups2, ua2, t0_init, back), fm.temp_data, fm.data_count, init_vals)
                    Y = fm.funcion_fiteo_refl_klm(fm.temp_data,popt[0],popt[1],t0_init,popt[2])
                elif ups2_fixed == 1 and t0_fixed == 0:
                    print str(type_fit) + " Semi-infinite, ups2 fixed"
                    popt, pcov = optimize.curve_fit(lambda t, ua2, t0, back: fm.funcion_fiteo_refl_klm(t, ups2_init, ua2, t0, back), fm.temp_data, fm.data_count, init_vals)
                    Y = fm.funcion_fiteo_refl_klm(fm.temp_data,ups2_init,popt[0],popt[1],popt[2])
                elif t0_fixed == 1 and ups2_fixed == 1:
                    print str(type_fit) + " Semi-infinite, t0 and ups2 fixed"
                    popt, pcov = optimize.curve_fit(lambda t, ua2, back: fm.funcion_fiteo_refl_klm(t, ups2_init, ua2, t0_init, back), fm.temp_data, fm.data_count, init_vals)
                    Y = fm.funcion_fiteo_refl_klm(fm.temp_data,ups2_init,popt[0],t0_init,popt[1])
            print "Fitted values: ", popt
            norm = ((Y-fm.data_count)**2).sum()           
            print "Norm: ", norm
			...
			self.ResultStore.append([index, ntpath.basename(filename), file_result, "Slab", popt[0], popt[1], t0_init, 0., 0., 0., norm])
                ...
                if t0_fixed == 0 and ups2_fixed == 0:
		    if type(pcov) != float: #Check for Inf variances (it pcov is float and not a list, it is probably Inf). They make the append fail.
                    	self.ResultStore.append([index, ntpath.basename(filename), file_result, "Semi_Inf", popt[0], popt[1], popt[2], math.sqrt(pcov[0,0]), math.sqrt(pcov[1,1]), math.sqrt(pcov[2,2]), norm])
		    else:
			self.ResultStore.append([index, ntpath.basename(filename), file_result, "Semi_Inf", popt[0], popt[1], popt[2], 0., 0., 0., norm])
                elif t0_fixed == 1 and ups2_fixed == 0:
		    if type(pcov) != float: #Check for Inf variances (it pcov is float and not a list, it is probably Inf). They make the append fail.
		    ...
		    if type(pcov) != float: #Check for Inf variances (it pcov is float and not a list, it is probably Inf). They make the append fail.
                    	self.ResultStore.append([index, ntpath.basename(filename), file_result, "Semi_Inf", ups2_init, popt[0], popt[1], math.sqrt(pcov[0,0]), math.sqrt(pcov[1,1]), math.sqrt(pcov[2,2]), norm])
		    else:
			self.ResultStore.append([index, ntpath.basename(filename), file_result, "Semi_Inf", ups2_init, popt[0], popt[1], 0., 0., 0., norm])
                elif t0_fixed == 1 and ups2_fixed == 1:
		    if type(pcov) != float: #Check for Inf variances (it pcov is float and not a list, it is probably Inf). They make the append fail.
                    	self.ResultStore.append([index, ntpath.basename(filename), file_result, "Semi_Inf", ups2_init, popt[0], t0_init, math.sqrt(pcov[0,0]), math.sqrt(pcov[1,1]), 0., norm])#math.sqrt(pcov[2,2])
		    else:
			self.ResultStore.append([index, ntpath.basename(filename), file_result, "Semi_Inf", ups2_init, popt[0], t0_init, 0., 0., 0., norm])           
            ...
            if t0_fixed == 0 and ups2_fixed == 0:            
                ...
                results.setdefault(index, []).append(fm.data_count)
                results.setdefault(index, []).append(fm.temp_data)
            
            if t0_fixed == 1 and ups2_fixed == 0:            
                results.setdefault(index, []).append(ntpath.basename(filename))         
                results.setdefault(index, []).append(popt[0]) 
                results.setdefault(index, []).append(popt[1])
            ...
            if ups2_fixed == 1 and t0_fixed == 0:            
                results.setdefault(index, []).append(ntpath.basename(filename))         
                results.setdefault(index, []).append(ups2_init) 
                results.setdefault(index, []).append(popt[0])
                results.setdefault(index, []).append(popt[1])            
                results.setdefault(index, []).append(popt[2])
                results.setdefault(index, []).append(norm)            
            ...
            if t0_fixed == 1 and ups2_fixed == 1:            
                results.setdefault(index, []).append(ntpath.basename(filename))         
                results.setdefault(index, []).append(ups2_init) 
                results.setdefault(index, []).append(popt[0])
                results.setdefault(index, []).append(t0_init)            
